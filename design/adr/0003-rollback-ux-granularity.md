# ADR-0003: Rollback UX granularity
...
Is declarative rollback (§13.5) single-component level or manifest level? Whether the frontend offers a "per-component rollback" option.
...
This ADR solidifies "rollback UX granularity" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §11 open issue and is still open.
